...
if __name__ == '__main__':
    ...
    for pdf in pdfs:
        ...
        # os.system("cmd")
        command_str_rm = r"rm -rf /house/yuanmingcai/kantan/yolov7/runs/detect/"
        os.system(command_str_rm)
        ...
        # 将预测结果复制到本地
        #判断本地路径中是否存在当前pdf对应的预测结果，如果存在则删除，因为需要复制新的标签文件过来
        label_dir_path = os.path.join(Predicted_Labels_path, pdf_name) #Predicted_Labels_path = r"./All_Label/Labels"
        if os.path.exists(label_dir_path):
            shutil.rmtree(label_dir_path)
        os.mkdir(label_dir_path)

        #再把标签文件一个一个复制粘贴过来
        # yolo预测结果存放位置yolo_label_dir
        yolo_label_dir = r"/house/yuanmingcai/kantan/yolov7/runs/detect/exp/labels"
        # yolo_detection_label_result = os.path.join(yolo_label_dir,pdf_path)
        labels = os.listdir(yolo_label_dir)
        for label in labels:
            label_file = os.path.join(yolo_label_dir,label)
